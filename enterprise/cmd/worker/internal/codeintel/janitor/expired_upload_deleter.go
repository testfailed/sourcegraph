...
func (d *expiredUploadDeleter) Handle(ctx context.Context) error {
	...
	count, err := tx.SoftDeleteOldUploads(ctx)
	if err != nil {
		return errors.Wrap(err, "SoftDeleteOldUploads")
	}
	if count > 0 {
		log15.Debug("Deleted expired upload records", "count", count)
		d.metrics.numUploadRecordsRemoved.Add(float64(count))
	}

	return nil
}

func (d *expiredUploadDeleter) HandleError(err error) {
